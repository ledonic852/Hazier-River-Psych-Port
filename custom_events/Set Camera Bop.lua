...
function onEvent(eventName, value1, value2, strumTime)
    if eventName == 'Set Camera Bop' then
        -- Sets up the data for the rate and removes any empty space in the string.
        local rateData = stringSplit(value1, ',')
        for i = 1, #rateData do
            rateData[i] = stringTrim(rateData[i])
        end
        ...
        if rateData[1] == '' then
            ...
            rate = tonumber(rateData[1])
        end
        -- How offset is the rate between camera bops
        if rateData[2] == nil then
            rateOffset = 0
        else
            rateOffset = tonumber(rateData[2])
        end
        ...
        setOnLuas('cameraZoomRateOffset', rateOffset)
...
function onStepHit()
    if not isRunning('custom_events/Set Camera Zoom') then
        if cameraZoomRate > 0 and cameraZoomOnBeat == true then
            if getProperty('camGame.zoom') < 1.35 and (curStep + (cameraZoomRateOffset * 4)) % (cameraZoomRate * 4) == 0 then
